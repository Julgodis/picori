...
use crate::{StringEncodingError, ensure};
...
pub trait ShiftJis2004 {
    fn from_shift_jis_2004(input: &[u8]) -> Result<String, PicoriError>;
}
...
impl ShiftJis2004 for String {
    fn from_shift_jis_2004(input: &[u8]) -> Result<String, PicoriError> {
        ...
        while i < input.len() {
            ...
            ensure!(i < input.len(), StringEncodingError::MissingData("another byte required"));
            ...
            return Err(StringEncodingError::InvalidCodepoint(code).into());
        }
        Ok(output)
    }
}
